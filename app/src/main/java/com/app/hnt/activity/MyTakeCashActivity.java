...
import com.app.hnt.model.MyTakeCashModel;
...
public class MyTakeCashActivity extends BaseActivity {
    ...
    List<MyTakeCashModel.ListBean> list = new ArrayList<>();
    CommonAdapter<MyTakeCashModel.ListBean> mAdapter;
    int page = 1;
    ...
    private void requestList(Map<String, String> params) {
        OkhttpUtil.okHttpGet(URLs.MyTakeCash, params, headerMap, new CallBackUtil<MyTakeCashModel>() {
            @Override
            ...
            public void onResponse(MyTakeCashModel response) {
                ...
                if (page > 1) {
                    //加载更多
                    List<MyTakeCashModel.ListBean> newlist = new ArrayList<>();
...
//                        adapter.notifyItemRangeInserted(list.size() - 1, newlist.size());
                    ...
                    if (list != null && list.size() > 0) {
                        showContentPage();
                        mAdapter = new CommonAdapter<MyTakeCashModel.ListBean>(MyTakeCashActivity.this,
                                R.layout.item_mytakecash, list) {
                            @Override
                            protected void convert(ViewHolder holder, MyTakeCashModel.ListBean model, int position) {
                                holder.setText(R.id.textView1,model.getStatus_title());
                                holder.setText(R.id.textView2,model.getInput_money());
