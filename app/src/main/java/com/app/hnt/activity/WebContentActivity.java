...
import com.app.hnt.model.TxtModel;
...
//        webView.loadUrl(url);//加载web资源
    ...
    protected void initData() {
        ...
        if (url!=null && !url.equals(""))
            ...
            params.clear();
            params.put("vkey", vkey);//regUser 表示用户注册协议 regPrivate 表示用户隐私协议
            requestWebURL(params);
        }

    }

    private void requestWebURL(Map<String, String> params) {
        OkhttpUtil.okHttpPost(URLs.WebURL, params, headerMap, new CallBackUtil<TxtModel>() {
            @Override
            public TxtModel onParseResponse(Call call, Response response) {
                return null;
            }
            ...
            @Override
            public void onResponse(TxtModel response) {
//                hideProgress();
//                webView.loadUrl(url);//加载web资源
                titleView.setTitle(response.getTitle());

                //适配屏幕
                StringBuilder sb = new StringBuilder();
                sb.append(getHtmlData(response.getV_details()));
                //加载HTML代码
    ...
    private void take() {
        ...
        final Intent captureIntent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);//打开相机
        ...
        WebContentActivity.this.startActivityForResult(chooserIntent, FILECHOOSER_RESULTCODE);
    }
